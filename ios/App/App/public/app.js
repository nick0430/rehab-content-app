...
// Web（http://localhost:3000）用相對路徑 /api/...
// iOS App（capacitor://...）改打 Render
...
async function fetchListFromServer() {
  try {
    ...
    const res = await fetch(`${API_BASE}/api/contents?${params.toString()}`);
    if (!res.ok) {
      if (statusEl) statusEl.textContent = `載入列表失敗（${res.status}）`;
      return;
    }
    ...
    if (!res.ok) {
      if (statusEl) {
        statusEl.textContent =
          res.status === 404 ? "找不到這則內容。" : `載入內容失敗（${res.status}）`;
      }
      return;
    }
